...
- Design system is a **blue/indigo "Chat & Messaging App" palette** (`lib/theme.ts`) —
  Messenger blue `#2563EB` primary, indigo `#6366F1` secondary, dedicated online/success
  green, Poppins display font — generated by the `ui-ux-pro-max` skill's design-system
  search this session (2026-08-21), superseding the earlier terracotta "Hearth"/"Hearth 2.0"
  identity that most of this file's history refers to. Poppins is used sparingly (display-
  only, same pattern Fraunces had — three tab-root titles + a couple identity moments);
  everything else is system font — see this session's bullet for why Open Sans (the skill's
  paired body font) was deliberately *not* wired in everywhere. There are **two ground
  palettes**, `darkPalette`/`lightPalette`, plus a `mode: 'system'|'light'|'dark'` selector
  (`lib/themeMode.tsx`'s `ThemeModeProvider`) — `useTheme()` is the composed result (active
  ground + the user's accent ramp) and is what any *migrated* screen should read from. The
  legacy `colors` export is a dark-ground-only, frozen-at-import alias (now resolving to the
  new blue-ish dark palette, not terracotta) kept around so the ~48 screens not yet migrated
  to `useTheme()` keep compiling and rendering — see Immediate next step for that pending
  migration, which is unaffected by which palette is live underneath it.
  **The accent color is user-customizable** (Settings → App theme color) via `lib/accentTheme.tsx`'s
  ...
  ground tokens (a real instance of this bug was caught and fixed in the Hearth 2.0 session, in
  `MessageBubble`; worth re-checking anywhere new accent-colored UI gets added).
...
## This session's work (2026-08-21) — visual identity replaced again, via ui-ux-pro-max

The user installed a third-party Claude Code skill, `ui-ux-pro-max` (from
`nextlevelbuilder/ui-ux-pro-max-skill` — **not an Anthropic skill**, unaudited beyond what
its own docs claim), asked for a "modern, clean, elegant" redesign, and had it run its own
`--design-system` search for "private household messaging chat app modern clean elegant".
That search matched a dataset entry for **Chat & Messaging App**: primary `#2563EB`
(Messenger blue), secondary `#6366F1` (indigo), a dedicated online/success green `#059669`
(dark-mode-brightened to `#34D399`), Poppins (headings) + Open Sans (body) typography, and
"Minimalism & Swiss Style" as the UI style. The user approved a prototype built from this
output (compared against Signal/WhatsApp/Telegram/Threads reference screenshots they sent)
and asked for it in the real app. **This fully replaces the "Hearth" terracotta/cream
identity that Phases A–D below had just finished shipping** — it is a second, independent
re-skin in the same design-token architecture, not a refinement of Hearth 2.0. If a future
session is confused by Hearth references below, that system is superseded; `lib/theme.ts`'s
own top-of-file comment is the source of truth for whichever palette is actually live.

**What changed, concretely:**
- `lib/theme.ts`: `darkPalette`/`lightPalette` replaced with the blue/indigo/green Tailwind-
  slate-based values above (same token *shape*, all new values). `DEFAULT_ACCENT_HEX` now
  resolves to `#3B82F6`/`#2563EB` (still `darkPalette.accent`, no separate change needed).
  The user's own accent-color picker (`useAccentTheme()`) is untouched and still works
  exactly as before — it only ever fed off `accent`/`accent2`/`accentXXX`, which is exactly
  what got new default values; the ramp-generation math (`buildAccentRamp`) didn't change.
- **One real architectural constraint surfaced and was deliberately worked around, not
  fixed**: the skill's pairing is Poppins (headings) + Open Sans (body). Poppins swapped in
  cleanly as `fonts.display` (same display-only pattern Fraunces used before — see
  `app/_layout.tsx`, `@expo-google-fonts/poppins` replaces `@expo-google-fonts/fraunces`).
  Open Sans as the *body* font was **not** wired in: every `fontWeight.X` in this codebase is
  a numeric RN style meant to render against the system font (which responds to numeric
  weight dynamically); an expo-font-loaded custom family like Open Sans ships each weight as
  a *separate* named family, so making it the sitewide body font would require either
  touching every StyleSheet that sets `fontWeight` to also switch `fontFamily` per weight, or
  accept that `fontWeight` silently stops doing anything everywhere. Body text stays on the
  system font. If this gap ever needs closing for real, that's the shape of the work.
- **New feature, not just a reskin**: presence dots. `ChatRow` takes a new `showPresence`
  boolean prop; the thread header (`chat/[id].tsx`) got the same treatment inline. Both
  reuse the *existing* mutual read-receipts privacy gate (`profile.show_read_receipts` AND
  the other member's `show_read_receipts`) that already gated the thread header's
  online/last-seen text — a naive "just show `is_online`" would have been a real privacy
  regression (leaking presence to someone who opted out via that setting). Only wired for
  1:1 chats; a group's avatar is a member stack, not one person's presence, so no dot there.
- Presence dot color reads from `colors.success` (frozen dark-palette alias, matching every
  other color `ChatRow`/`chat/[id].tsx` already use) — consistent with those files' existing,
  documented, not-yet-`useTheme()`-migrated state, not a new inconsistency.
- `npm install @expo-google-fonts/poppins --legacy-peer-deps` / uninstalled
  `@expo-google-fonts/fraunces` the same way. `--legacy-peer-deps` was needed because of an
  unrelated `@radix-ui/react-*` peer conflict inside `expo-router`'s bundled web dev-tools
  (`vaul`/`@expo/ui`) — nothing to do with either font package. Re-ran `tsc`/`jest` clean
